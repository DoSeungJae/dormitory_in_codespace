...
function EmailVerifyForRecoveryForm({setRecoveryDetails}){
    const {isOpen, openModal, closeModal}=useContext(ModalContext);
    
    const[sent,setSent]=useState(false);
    const[email,setEmail]=useState("");
    const[message, setMessage]=useState("비밀번호를 변경하기 위해 이메일을 인증해주세요.");
    ...
    const authenticateVerifyCode = async () => {
        ...
        const path=`${process.env.REACT_APP_HTTP_API_URL}/email/authenticateCode/token`;
        const headers={
            Email:`${email}`,
...
export default EmailVerifyForRecoveryForm;
